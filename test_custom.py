...
def find_best_checkpoint(checkpoint_dir='results/checkpoints'):
    ...
    Searches both root and per-method subdirectories (e.g. DMJO/full/).
    """
    candidates = [
        os.path.join(checkpoint_dir, 'DMJO', 'full', 'best_model.pt'),
        os.path.join(checkpoint_dir, 'MAPPO', 'full', 'best_model.pt'),
        os.path.join(checkpoint_dir, 'best_model.pt'),
        os.path.join(checkpoint_dir, 'DMJO', 'full', 'final_model.pt'),
        os.path.join(checkpoint_dir, 'MAPPO', 'full', 'final_model.pt'),
        os.path.join(checkpoint_dir, 'final_model.pt'),
    ]
    for c in candidates:
        if os.path.exists(c):
            return c, f"{os.path.basename(c)} ({os.path.relpath(c, checkpoint_dir)})"

    step_files = glob.glob(os.path.join(checkpoint_dir, '**', 'model_step*.pt'), recursive=True) + \
                 glob.glob(os.path.join(checkpoint_dir, 'model_step*.pt'))
    if step_files:
        def _step(p):
        ...
        chosen = step_files[-1]
        return chosen, f'{os.path.basename(chosen)} ({os.path.relpath(chosen, checkpoint_dir)})'
